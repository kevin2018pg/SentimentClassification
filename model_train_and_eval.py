# -*- coding: utf-8 -*-
# @Time    : 2020/4/14 20:18
# @Author  : Kevin
# @File    : model_train_and_eval.py
# @Version : python 3.6
# @Desc    : 训练验证
...
def trains(args, train_dataset, eval_dataset, model, fold_num=None):
    ...
    # 只要参数包含bert_params其中之一且参数不包含no_decay其中之一，就进行权重衰减
...
def evaluate(args, model, eval_dataset, is_test=False):
    ...
    model.train()
    if is_test:
        report = classification_report(y_true=out_label_ids, y_pred=preds, target_names=eval_dataset.get_labels(),
                                       digits=4)
...
def test(args, model, test_dataset):
    # test
    model.eval()
    test_loss, test_acc, label_f_score, test_report, test_confusion = evaluate(args, model, eval_dataset=test_dataset,
                                                                               is_test=True)
